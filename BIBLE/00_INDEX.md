...
- **Jalon 0.0.3 « Individus » DEMARRE, tranche 1 faite (2026-09-01)** : le premier souvenir. Schema v7. Nouveau module `cognition.rs` (`Mind`, `Memory`, `MemoryKind`, `Shock`), `Entity.mind: Option<Box<Mind>>` + `Entity.last_shock`, phase 5c `cognition_phase` (eveil, entretien memoire, retombee, sans RNG), biais memoire dans la decision (phase 2/3), `CognitionCfg` (`[cognition]`), `EventKind::AgentAwoke` / `AgentLapsed`, `WorldState::agent_stats`, `genesis-view` (`EntityView.agent`, `WorldStats.agents_alive` / `mean_memories`, `SeriesRow.agents_alive`, `VIEW_VERSION` 6), lecteur (panneau Agents, anneau), CLI `lives.jsonl` + sortie, tests `agents_awake_and_remember` + `agent_promotion_is_reversible`. Detail : `05_COGNITION.md`. `worlds/w2` regenere. Reste 0.0.3 : besoins, personnalite heritee, souvenirs ancres, page biographie, modele de comportement complet ; et le modele de temps a deux horloges (`04_SIMULATION.md`) pour le temps evolutif.

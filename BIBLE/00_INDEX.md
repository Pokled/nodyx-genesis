...
| **Répulsion douce des bords (0.0.3).** Le bord du monde est un cul-de-sac : la chimiotaxie (le troupeau remonte le gradient de fertilité) plus le biais mémoire entassent la population contre la paroi la plus riche, et `solitude` resserre l'amas. `sim.rs::edge_correct` décale la cible de déplacement de **toutes** les entités vers l'intérieur quand elles sont dans la bande `edge_margin` (10 cases) d'un bord, proportionnellement à la pénétration, force `edge_push` (0.6). Symétrique sur les 4 bords : ce n'est **pas** une force codée « vers le bas », le sens de la dérive vient du relief de fertilité propre à chaque graine. Aucun tirage RNG, `edge_push = 0` rend le monde byte-identique. Config `[world]`. | LOCKED (2026-09-01, réponse à une observation utilisateur). A/B graine 1 : entités collées à la paroi au tick 6000 (premier effondrement) 33 % -> 6 %, centre de masse final décalé ~6 cases vers l'intérieur, population inchangée. Effet de bord : la mémoire des agents s'enrichit (population plus étalée = lieux de péril plus variés, moyenne 1,5 -> 5,2 souvenirs). Test `edge_push_spreads_the_herd`. |
| Génome à **9 traits** : 7 de corps + 2 de personnalité (`caution`, `curiosity`, hérités, 0.0.3 tranche 5). `genome.rs::N_TRAITS` ; `SPECIES_TRAITS = 7` (la signature d'espèce ne porte que sur le corps). Les instantanés d'un schéma antérieur ne se rechargent plus, on régénère les `worlds/`. **Le monde de référence `worlds/w2` passe de la graine 3 à la graine 1 au schéma v10** (le décalage RNG des 2 traits a rendu la graine 3 non viable). | LOCKED. Schéma : v3 (entités `Vec`), v4 (matière structurelle), v5 (cellules), v6 (`next_event_seq` + traçabilité `Watch`), v7 (cognition : `Entity.mind`, `Entity.last_shock`), v8 (`MemoryKind::Witnessed`, ancré sur l'`EntityDied`), v9 (`Mind.needs` : faim, peur, solitude), v10 (génome à 9 traits, personnalité héritée). |
...
- **Jalon 0.0.3 « Individus » DEMARRE, tranches 1 a 5 faites (2026-09-01)** :
  ...
  - **Correction du piege de coin (2026-09-01).** `sim.rs::edge_correct` : repulsion douce des bords contre l'entassement du troupeau contre la paroi (chimiotaxie plus memoire). Config `[world]` `edge_margin` / `edge_push`, symetrique, sans RNG (`edge_push = 0` = byte-identique). A/B graine 1 : au bord au premier effondrement 33 % -> 6 %. Test `edge_push_spreads_the_herd`.
  - Detail : `05_COGNITION.md`. **`worlds/w2` = graine 1 depuis v10** (graine 3 non viable apres le decalage RNG). Reste 0.0.3 : modele de comportement lisible, de-simulation de la biologie ; et le modele de temps a deux horloges (`04_SIMULATION.md`).

...
| Génome à **7 traits** (schema v2, 2026-09-01) : ajout de `cohesion` (tendance à s'agréger, évoluable). `genome.rs::N_TRAITS`. Les instantanés d'un schéma antérieur ne se rechargent plus, on régénère les `worlds/`. | LOCKED. Schéma : v3 (entités en `Vec`), v4 (matière structurelle sur `WorldState`), v5 (cellules : `WorldState.cells`, `Entity.cell_id`), v6 (`WorldState.next_event_seq` : `seq` d'événement attribué à la création ; champs de traçabilité sur `Watch`), v7 (cognition : `Entity.mind`, `Entity.last_shock`), v8 (`MemoryKind::Witnessed` : souvenir de mort vue, ancré sur l'`EntityDied`), v9 (`Mind.needs` : faim, peur, solitude). |
...
- **Jalon 0.0.3 « Individus » DEMARRE, tranches 1 a 4 faites (2026-09-01)** :
  ...
  - Detail : `05_COGNITION.md`. `worlds/w2` regenere. Reste 0.0.3 : personnalite heritee, modele de comportement complet ; et le modele de temps a deux horloges (`04_SIMULATION.md`) pour le temps evolutif.

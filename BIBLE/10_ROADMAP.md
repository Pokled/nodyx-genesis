...
| 0.0.4 | Voix | Signaux, langage émergent, groupes, transmission. **Démarré :** t1 le premier signal (`WorldState.signals`, schéma v14) : un agent affamé émet une alarme, les agents proches ont un sursaut de peur, aucun lexique. A/B graine 1 : famine -10 %, diversité +37 % (la panique alerte et disperse). t2 l'appel (`Signal.kind`, schéma v17) : un agent qui mange bien sur une case riche lance un appel « bon coin », les agents proches qui décident infléchissent leur cible vers lui. A/B graine 1 : naissances +4 %, diversité +46 % (l'appel concentre sur les bons coins, brasse les lignées). Deux genres fixes, toujours aucun lexique. En parallèle, **les saisons** (`experiments/011`) : la capacité nourricière du sol oscille au fil de l'année, la population déborde puis reflue (5300 ↔ 9600), la disette rebrasse les lignées ; et une composante thermique décalée (schéma v18, trait `heat_tol`) ajoute un second stress annuel qui lève la diversité génétique de ~44 %. Le monde de référence n'est plus une ligne plate, et il a enfin l'environnement changeant que le sexué émergent attendait. | Deux populations isolées qui divergent linguistiquement | Un système de signes émerge-t-il sans qu'on code un lexique ? |

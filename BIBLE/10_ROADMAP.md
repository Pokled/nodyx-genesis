...
| 0.0.2 | Vie | 100 entités et plus, génome complet, sélection naturelle, statistiques, traçabilité causale, bascule molécule vers cellule. Faits : capacité de charge par la matière (`experiments/005`) ; bascule cellule étape 1 (`experiments/006`) ; sélection mesurée, série temporelle `series.jsonl` + graphe `series.html`. Reste : traçabilité causale (`Event.causes`) | Un graphe d'évolution génétique sur 10 000 générations, en direct | Le graphe existe (`series.html`) ; 10 000 générations demandent le modèle de temps à deux horloges (`04_SIMULATION.md`), non construit : un run de 60 000 ticks donne ~30 générations |
| 0.0.3 | Individus | Mémoire, personnalité, besoins, objectifs, relations, comportement. **Cible probante.** Toujours sans LLM | La première biographie auto-générée | Le comportement dépend-il vraiment du souvenir, ou est-ce cosmétique ? Substrat cognitif semé ou évolutif ? (voir `05_COGNITION.md`) |

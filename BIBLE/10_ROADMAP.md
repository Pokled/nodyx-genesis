...
| 0.0.3 | Individus | Mémoire, personnalité, besoins, objectifs, relations, comportement. **Cible probante.** Toujours sans LLM. **Démarré :** t1 le premier souvenir (mémoire qui biaise le déplacement, famine -15 %) ; t2 `lives.html`, la biographie auto-générée (gabarits, aucun LLM) ; t3 le souvenir ancré (`Memory.event_seq` pointe l'`EntityDied`, famine -33 % de plus) ; t4 les besoins (faim, peur, solitude pondèrent le comportement, famine -54 % de plus, la population meurt de vieillesse maintenant). Cumul : famine ~15 500 -> ~4 000. Reste : personnalité héritée, comportement complet | La première biographie auto-générée (`lives.html` : souvenirs tracés jusqu'à leur événement, jauges et tempérament) | Le comportement dépend-il vraiment du souvenir, ou est-ce cosmétique ? Substrat cognitif semé ou évolutif ? (voir `05_COGNITION.md`) |

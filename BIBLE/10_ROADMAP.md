...
| 0.0.3 | Individus | Mémoire, personnalité, besoins, objectifs, relations, comportement. **Cible probante.** Toujours sans LLM. **Démarré :** tranche 1, le premier souvenir (éveil Entity vers Agent, mémoire qui biaise le déplacement, morts par famine -15 %) ; tranche 2, `lives.html`, la première biographie auto-générée (prose de gabarits, aucun LLM) ; tranche 3, le souvenir ancré (voir mourir un des siens, `Memory.event_seq` pointe l'`EntityDied`, morts par famine -33 % de plus). Reste : besoins, personnalité héritée, comportement complet | La première biographie auto-générée (`lives.html`, en place ; souvenirs tracés jusqu'à leur événement) | Le comportement dépend-il vraiment du souvenir, ou est-ce cosmétique ? Substrat cognitif semé ou évolutif ? (voir `05_COGNITION.md`) |
